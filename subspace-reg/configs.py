...
def parse_option_eval():
    ...
    parser.add_argument('--model_path', type=str, default='/dumped/backbones/continual/resnet18/2/resnet18_last.pth', help='absolute path to .pth model')
    parser.add_argument('--save_model_path', type=str, default='', help='path to save model')

    # dataset
    parser.add_argument('--dataset', type=str, default='cub200', choices=['miniImageNet', 'tieredImageNet','cub200',
                                                                                'CIFAR-FS', 'FC100'])
    ...
    parser.add_argument('--n_base_aug_support_samples', default=0, type=int,
                        help='The number of augmented samples for each meta test sample')
    parser.add_argument('--n_base_support_samples', default=1, type=int,
    ...
    parser.add_argument('--memory_replay', type=int, default=1,
...
def parse_option_supervised():
    ...
    # dataset
    parser.add_argument('--model', type=str, default='efficientnet0', choices=model_pool)
    parser.add_argument('--dataset', type=str, default='cub200', choices=['miniImageNet', 'tieredImageNet','cub200',
                                                                                'CIFAR-FS', 'FC100'])
    parser.add_argument('--transform', type=str, default='A', choices=transforms_list)
 ...
 #############################       
    opt.save_final_folder = opt.save_model_path # os.path.join(opt.model_path, opt.model_name)
    if not os.path.isdir(opt.save_final_folder):
        os.makedirs(opt.save_final_folder)
