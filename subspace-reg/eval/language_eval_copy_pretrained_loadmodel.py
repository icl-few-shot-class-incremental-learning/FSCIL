...
################저장한 모델 불러오기##################
    ckpt = torch.load(opt.save_model_path)

    net = EfficientNet.from_pretrained('efficientnet-b0',num_classes = 200)
    net._fc.bias = None
    ...
    for idx in range(iter_num):
        ...
        test_acc, test_acc_top5, test_loss, query_ys_pred = validate(novel_query_collection,
                                                                                ...
                                                                                net, criterion, opt, epoch)
                                                                                  
        # while stop_condition:
        #     #_fc -> model classifier , weight의 require_grad 설정\
        #     freeze_backbone_weights(net, opt, epoch, exclude=["_fc","fc"])
        ...
        #         for i,j in enumerate(novel_query_collection_id):
        #             length+=1
        #             if novel_query_collection_id[i]==query_ys_pred[-1][i]:
        #                 class_acc+=1
        #             if i!=len(query_ys_id)-1:
        #                 if j!=query_ys_id[i+1]:
        #                     nov_acc.append(class_acc/length)
        ...
        #         nov_acc = [round(i,2) for i in nov_acc]
        ...
        state = {
            ...
            # if opt.n_gpu <= 1 else net.module.state_dict(),
        }
        if opt.continual:
            state['training_classes'] = val_train.dataset.basec_map
            state['label2human'] = val_train.dataset.label2human
        save_file = (opt.save_model_path +  'ckpt_iter_{idx}.pth').format(idx=idx)
