...
def create_model(name, n_cls, opt, vocab=None, dataset='miniImageNet'): #name이 backbone임
    ...
    if dataset == 'miniImageNet' or dataset == 'tieredImageNet':
        if name.endswith('v2') or name.endswith('v3'):
            ...
            model = model_dict[name](avg_pool=True, drop_rate=0.1, 
                                     ...
                                     vocab=vocab, opt=opt) #TODO model이 resnet18로
        elif name.startswith('efficientnet'):
            model=model_dict[name](num_classes=n_cls,vocab=vocab, opt=opt)             
    ...
    elif dataset=='cub200':#############데이터셋 이름 바꾸기
...
def get_embeds(embed_pth, vocab, dim=300):
    '''
