...
#                                     num_workers=opt.num_workers)
        ...
        train_trans, test_trans = transforms_options[opt.transform]
        train_loader = DataLoader(cub200(args=opt, base_sess = True, train = True, transform=train_trans,index = 100, index_path = 1),
                                  batch_size=opt.batch_size, shuffle=True, drop_last=True,
                                  num_workers=opt.num_workers)

        val_loader = DataLoader(cub200(args=opt, base_sess = True, train=False, transform=test_trans,index = 100, index_path = 1),
                                batch_size=opt.batch_size // 2, shuffle=False, drop_last=False,
                                num_workers=opt.num_workers // 2)
        if opt.use_trainval:
            n_cls = 100
...
#         vocab_val = [name for name in meta_valloader.dataset.label2human if name != '']
    ...
    # model
    if opt.model == 'efficientnet0':
        model = EfficientNet.from_pretrained('efficientnet-b0',num_classes = 100)
        model.add_prompt()
        model._fc.bias = None
    else:
        model = resnet18test.resnet18(pretrained=True)
        model.fc.bias = None
    

    #model = create_model(opt.model, n_cls, opt, vocab=vocab,dataset = 'cub200')
...
#         print("Before Epoch {}".format(epoch))
        ...
        if epoch % opt.save_freq == 0:
            ...
            save_file = (opt.model_path +  'ckpt_epoch_{epoch}.pth').format(epoch=epoch)
    ...
    if opt.continual:
        state['training_classes'] = train_loader.dataset.basec_map
        state['label2human'] = train_loader.dataset.label2human
    save_file = (opt.model_path + '{}_last.pth').format(opt.model)
    torch.save(state, save_file)
