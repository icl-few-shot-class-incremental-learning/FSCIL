...
def get_vocabs(base_loader=None, novel_loader=None, query_ys=None):
    ...
    vocab_base = None
    if base_loader is not None:
        label2human_base = base_loader.dataset.label2human[0:100]
        vocab_base  = [name for name in label2human_base if name != '']
        vocab_all  += vocab_base
...
def drop_a_dim_t(data): #TODO why do we need this in the first place?
    support_xs, support_ys, *_ = data
    batch_size,height, width, channel = support_xs.size()
    support_xs = support_xs.view(-1, height, width, channel)
    support_ys = np.array(support_ys) # TODO
    return (support_xs, support_ys)

def drop_a_dim(sup_xs, sup_ys, qu_xs, qu_ys): #TODO why do we need this in the first place?
    support_xs, support_ys, query_xs, query_ys = sup_xs,sup_ys,qu_xs,qu_ys
    batch_size, height, width, channel = support_xs.size()
    support_xs = support_xs.view(-1, height, width, channel)
