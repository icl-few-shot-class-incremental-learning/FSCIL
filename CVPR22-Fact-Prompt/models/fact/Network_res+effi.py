...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        if self.args.dataset == 'cub200':
            ...
            self.encoder2 = resnet18(True, args)
        ...
        ## added
        self.fc2 = nn.Linear(512, self.pre_allocate, bias=False)
    ...
    def pre_encode(self,x):
        ...
        if self.args.dataset in ['cifar100','manyshotcifar']:
            ...
            x = self.encoder2.conv1(x)
            x = self.encoder2.bn1(x)
            x = self.encoder2.relu(x)
            x = self.encoder2.maxpool(x)
