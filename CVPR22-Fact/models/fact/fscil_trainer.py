...
class FSCILTrainer(Trainer):
    def __init__(self, args):
        ...
        if self.args.model_dir is not None:
            ...
            self.path = './checkpoint/cub200/fact/ft_cos-avg_cos-data_init-start_0/Cosine-Epo_100-Lr_0.0050Bal0.01-LossIter50-T_16.00'
            self.best_model_dict = self.model.load_state_dict(torch.load(self.path+'/session0_max_acc.pth')['params'])
    ...
    def train(self):
        ...
        #print('####',self.set_save_path)
        #path = './checkpoint/cub200/fact/ft_cos-avg_cos-data_init-start_0/Cosine-Epo_1-Lr_0.0050Bal0.01-LossIter50-T_16.00'
        path = './checkpoint/cub200/fact/ft_cos-avg_cos-data_init-start_0/Cosine-Epo_100-Lr_0.0050Bal0.01-LossIter50-T_16.00'
        #self.model.load_state_dict(torch.load(path+'/session0_max_acc.pth'))
        #self.model.load_state_dict(dict(params=torch.load(path+'/session0_max_acc.pth')))
        self.model.load_state_dict(torch.load(path+'/session0_max_acc.pth')['params'])
        # torch.save(dict(params=self.model.state_dict()), save_model_dir)
        print('$$$$', args.start_session, args.sessions)
        for session in range(1, args.sessions):
            ...
            optimizer = torch.optim.AdamW(self.model.parameters(), self.args.lr_base, 
                                    weight_decay=self.args.decay)
            optimizer.load_state_dict(torch.load(path+'/optimizer_best.pth'))
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.args.epochs_base)
